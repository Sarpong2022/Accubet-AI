...
# --- config ---
st.set_page_config(page_title="NBA Statistics using Machine Learning",
                   page_icon=":basketball:",
                   layout="wide"
                   )
df = pd.read_csv('NBA_Player.csv', delimiter=";", encoding="latin-1", index_col=0)
df.columns = df.columns.str.replace(' ', '_')
df.drop(df[df['Tm'] == 'TOT'].index, inplace=True)

# --- sidebar ---
st.sidebar.header("Please Select Teams:")
team = st.sidebar.selectbox(
    "Select the Teams Here :",
...
# --- main ---
st.title(":basketball: NBA Latest Machine Learning Based Statistics  ")
st.markdown("##")
